...
$updateanypost =has_capability('local/greetings:updateanymessage', $context);
$updateownpost=has_capability('local/greetings:updateownmessage', $context);
    ...
    foreach ($messages as $m) {
        ...
        if ($deleteanypost || ($deletepost && $m->userid == $USER->id)) {
            ...
            echo html_writer::link(
                ...
                array('role' => 'button', 'aria-label' => get_string(''), 'title' => get_string(''))
            );
            echo html_writer::end_tag('p');
        }
        ...
        if ($updateanypost || ($updatepost && $m->userid == $USER->id)) {
            echo html_writer::start_tag('p', array('class' => 'card-footer text-center'));
            echo html_writer::link(
                new moodle_url(
                    '/local/greetings/index.php',
                ...
                $OUTPUT->pix_icon('t/edit', ''), // Displays the update icon (edit)
                array('role' => 'button', 'aria-label' => get_string(''), 'title' => get_string(''))
            );
            echo html_writer::end_tag('p');
        }
